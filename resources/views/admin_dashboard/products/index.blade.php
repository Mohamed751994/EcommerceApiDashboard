...
@section('Page_Title')  @lang('text.products-index') @endsection
    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex align-items-center">
                <h5 class="mb-0"> <i class="bi bi-grid-fill"></i>   @lang('text.products-index') <small> ({{$content->total()}})</small> </h5>
                @can('products-create')

                    <div class="ms-auto position-relative">
                        <a href="{{route('products.create')}}" class="btnIcon btn btn-outline-primary px-5"><i class="lni lni-circle-plus"></i> @lang('text.Create') </a>
                    </div>
                            ...
                            <td>
                                <div class="form-check form-switch">
                                    <input class="form-check-input customSliderCheckbox" type="checkbox"
                                           name="status" onchange="quickChange(this, '{{Product::class}}', '{{$con->id}}', 'status')" id="flexSwitchCheckChecked" @if($con->status) value="0" checked="" @else value="1" @endif>
                                </div>
                            </td>
                            <td>
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            ...
                            type: 'post',
                            data: {_token: '{{csrf_token()}}', ids:Ids,model:'{{Product::class}}' },
                            success: function(response) {
                                if(response.success)
                                {
                                    location.reload();
